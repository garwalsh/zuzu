...
**Live:** https://zuzu-orchestrator.onrender.com/dashboard —  add `?secret=<ZUZU_SHARED_SECRET>`
for live updates, or press **Demo Mode** to watch a full call fill the form with
no microphone.
...
| Everything else in `api/`, `mocks/` | Hand-written **to** the prompt specs |
...
two above were machine-generated, and the README should say so plainly.

Regeneration was attempted for the rest with the test suite as the gate: modules
whose generated version failed the suite were reverted automatically rather than
shipped. `i765_schema` failed that gate, which is exactly the outcome the gate
exists for. See [`docs/SPEC-CORRECTIONS.md`](docs/SPEC-CORRECTIONS.md) §5 for the
working local invocation and the model-catalog pitfall on this machine.
...
| 2. Dashboard + realtime + Demo Mode button | **done** |
| 3. mem0 returning-caller memory — the moat | **done** — 33 questions → 4 on the second call |
| 4. Inference lane: spoken-value normalisation + eligibility helper | not started |
| 5. ElevenLabs voice agent + widget | **done** — agent live, tools wired to the deploy |
| 6. Deploy | **done** — Render, one blueprint |
| 7. Layer 2: Redis, worker, 50-caller load test | not started |

**On Cerebras.** The playbook assigns it the speed lane. The account has no
credits (`402 Payment Required`), and the model the docs name, `llama-3.3-70b`,
does not exist on it (`404`). Rather than claim an integration that never ran,
the inference lane is written provider-agnostically against an OpenAI-compatible
interface: TokenRouter is the working default and Cerebras is a config flip if
credits arrive. **Zuzu does not claim the Cerebras track.**
